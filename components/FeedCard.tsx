...
function getCategoryLabel(category?: string | null): string {
  ...
  const labels: Record<string, string> = {
    ...
    computer_science: "コンピュータ科学",
    machine_learning: "機械学習",
    ai: "AI",
    quantum: "量子情報",
    robotics: "ロボティクス",
    cyber_security: "セキュリティ",
    climate: "気候変動",
    ecology: "生態学",
    energy: "エネルギー",
    geology: "地質学",
...
export function FeedCard({ item, sessionId, isActive, onLike, onSave }: FeedCardProps) {
  ...
  useEffect(() => {
    if (isActive && !viewTracked.current) {
      ...
      trackInteraction(sessionId, item, "view");
    }
  }, [isActive, sessionId, item]);
  ...
  const handleLike = useCallback(() => {
    ...
    if (newLiked) {
      trackInteraction(sessionId, item, "like");
      onLike?.(item.id);
    }
  }, [liked, sessionId, item, onLike]);
  ...
  const handleSave = useCallback(() => {
    ...
    if (newSaved) {
      trackInteraction(sessionId, item, "save");
      onSave?.(item.id);
    }
  }, [saved, sessionId, item, onSave]);
  ...
  return (
    ...
    >
      ...
      {/* Top bar */}
      <div className="absolute top-0 left-0 right-0 z-10 flex items-center justify-between px-5 pt-safe-top pt-4 pb-3 bg-gradient-to-b from-black/50 to-transparent">
