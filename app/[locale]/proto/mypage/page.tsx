...
import { ARTICLES, CATEGORY_STYLE, getCategoryLabel } from "../../../../lib/proto/mockData";
...
export default function MyPage() {
  const { saved, seen, streak, totalPoints, removeSaved } = useProtoStore();
  const [openArticle, setOpenArticle] = useState<Article | null>(null);
  const t = useTranslations("Proto.mypage");

  const items = saved.map((id) => ARTICLES.find((a) => a.id === id)).filter((a): a is Article => !!a);
