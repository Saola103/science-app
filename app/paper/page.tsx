  ...
  try {
    ...
    return {
      ...
      openGraph: {
        title,
        description: summary,
        url: `${appUrl}/paper?id=${encodeURIComponent(id)}`,
...
export default async function PaperSharePage({ searchParams }: Props) {
  const { id } = await searchParams;
  ...
  return <PaperShareClient paper={data} />;
}
